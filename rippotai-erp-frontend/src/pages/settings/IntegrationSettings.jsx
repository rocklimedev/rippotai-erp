...
} from "../../api/auth/authConnectors.api";
...
function GoogleLogo({ size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" aria-hidden="true">
      ...
      <path
        fill="#34A853"
        d="M24 47c6.3 0 11.6-2.1 15.5-5.7l-7.6-6c-2.1 1.4-4.8 2.3-7.9 2.3-6.1 0-11.2-4.1-13-9.6H3.2v6.1C7.1 41.8 14.9 47 24 47z"
      />
...
export default function IntegrationSettings() {
  ...
  /* Authorization                                                       */
  /*                                                                      */
  /* All three providers follow the same shape: an authenticated RTK      */
  /* Query call (sends the JWT via the Authorization header) returns an   */
  /* { authorizationUrl }, and only THEN do we navigate the browser to    */
  /* that URL. A raw window.location.assign() straight to our own        */
  /* JWT-protected backend route would not carry the Authorization        */
  /* header and would 401 — that was the bug in the old Microsoft flow.   */
  /* ------------------------------------------------------------------ */
  ...
  const connectedCount = providers.filter(
    (provider) => !!provider.status.data?.connected,
  ).length;
