...
export function BoqSummaryHeader({ boq, disabled }) {
  ...
  const clientName = boq.client_name || boq.project?.client?.name || "—";

  const location = boq.location || boq.project?.site_location || "—";

  const total = boq.final_total ?? boq.total_value ?? 0;

  const projectTotal = boq.project_total ?? 0;

  const miscAmount = boq.misc_amount ?? 0;

  return (
    <section className="bc-card p-6 md:p-8" data-testid="boq-summary-header">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1px_360px] gap-6 md:gap-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-2 mt-5 text-[13px]">
            <div>
              ...
              <div className="text-[#333333] mt-0.5">{clientName}</div>
            </div>
            ...
            <div>
              ...
              <div className="text-[#333333] mt-0.5">{location}</div>
            </div>
        ...
        <div className="md:pl-2">
          ...
          <div className="text-[11.5px] text-[#6B7B7C] mt-1">
            ...
            {" · "}
            {/* Miscellaneous {boq.misc_pct || 0}% {formatINR(miscAmount)} */}
          </div>
        </div>
      </div>
    </section>
